...
for TTY in $TTYS; do
    echo "$NAME is starting..." > "$TTY" || true
done
...
if [ -e /sys/firmware/efi/efivars/SecureBoot-8be4df61-93ca-11d2-aa0d-00e098032b8c ]; then
    ...
    if [ "$secure_boot_state" = 1 ]; then
        ...
        if [ -e /sys/firmware/efi/efivars/AuditMode-8be4df61-93ca-11d2-aa0d-00e098032b8c ]; then
            ...
            if [ "$audit_mode_state" = 1 ]; then
                for TTY in $TTYS; do
                    echo "\033[31m$NAME cannot boot while SecureBoot is in Audit Mode.\033[0m" > "$TTY" || true
                done
                sleep 3600
            fi
        fi
    ...
    if [ "$secure_boot_state" != 1 ]; then
        for TTY in $TTYS; do
            echo "\033[0;33mSecureBoot is disabled. $NAME will attempt to fall back to a less-secure boot logic.\033[0m" > "$TTY" || true
            SECURE_BOOT_DISABLED=true
        done
    fi
else
    for TTY in $TTYS; do
        echo "\033[31mUnable to determine SecureBoot state. $NAME will only boot on UEFI systems.\033[0m" > "$TTY" || true
    done
    sleep 3600
fi
...
if [ -e /sys/class/tpm/tpm0/tpm_version_major ]; then
    ...
    if [ "$tpm_version" != 2 ]; then
        for TTY in $TTYS; do
            echo "\033[31mDetected an unsupported v1.2 TPM; $NAME requires a v2.0 TPM. Please upgrade the TPM or disable it in the system's firmware and configure $NAME to use swtpm.\033[0m" > "$TTY" || true
        done
        sleep 3600
    fi
else
    for TTY in $TTYS; do
        echo "\033[0;33mNo TPM detected. $NAME will attempt to fall back to a less-secure swtpm implementation.\033[0m" > "$TTY" || true
        TPM_MISSING=true
    done
fi
...
if $SECURE_BOOT_DISABLED && $TPM_MISSING; then
    for TTY in $TTYS; do
        echo "\033[31m$NAME cannot boot if SecureBoot is disabled and no physical TPM is present.\033[0m" > "$TTY" || true
    done
    sleep 3600
fi
