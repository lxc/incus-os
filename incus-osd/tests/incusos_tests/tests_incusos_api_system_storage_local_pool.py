...
def TestIncusOSAPISystemStorageLocalPoolScrub(install_image):
    ...
    with IncusTestVM(test_name, test_image) as vm:
        ...
        if result["metadata"]["state"]["pools"][0]["last_scrub"]["examined_bytes"] <= 0:
            raise IncusOSException("expected a positive amount of examined bytes to be reported on the last scrub")

        if result["metadata"]["state"]["pools"][0]["last_scrub"]["to_examine_bytes"] <= 0:
            raise IncusOSException("expected a positive amount of to examine bytes to be reported on the last scrub")
