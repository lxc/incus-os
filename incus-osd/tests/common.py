...
def _download_images(require_prior_release_stable=True):
    os_name = ""
    ...
    with urllib.request.urlopen(IMAGES_SERVER + "/os/index.json") as url:
        ...
        for update in versions["updates"]:
            ...
            elif "stable" in update["channels"] or not require_prior_release_stable:
                prior_release = update
                break

        if prior_release is None:
            raise IncusOSException("need at least two published releases")
        ...
        for file in prior_release["files"]:
            if file["architecture"] == "x86_64":
                if file["type"] == "image-raw":
                    urls.append(IMAGES_SERVER + "/os" + prior_release["url"] + "/" + file["filename"])
    ...
    prior_image_img = os.path.join(os.getcwd(), os_name + "_" + prior_release["version"] + ".img")
