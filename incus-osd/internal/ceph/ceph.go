...
func InitializeCephCluster(ctx context.Context, config map[string]string) error {
	...
	cephConfigFiles, err := getCephClusterConfigFiles(ctx, cephControlContainerNames[0])
	if err != nil {
		return err
	}
...
func AddOSD(ctx context.Context, config map[string]string) error {
	...
	project, etag, err := incusClient.GetProject(projectName)
	if err != nil {
		return err
	}
	...
	err = incusClient.UpdateProject(projectName, project.ProjectPut, etag)
	if err != nil {
		return err
	}
