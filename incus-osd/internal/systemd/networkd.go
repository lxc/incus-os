package systemd

import (
	"context"
	"fmt"
	"os"
	"path/filepath"
	"strings"
)

type ConfigFile struct {
	Name     string
	Contents string
}

// GenerateNetworkConfiguration clears any existing configuration from /run/systemd/network/ and generates
// new config files from the supplied NetworkConfig struct.
func GenerateNetworkConfiguration(networkCfg NetworkConfig) error {
	// Remove any existing configuration.
	err := os.RemoveAll(SystemdNetworkConfigPath)
	if err != nil {
		return err
	}

	err = os.Mkdir(SystemdNetworkConfigPath, 0o750)
	if err != nil {
		return err
	}

	// Generate .link files.
	for _, cfg := range GenerateLinkFileContents(networkCfg) {
		err := os.WriteFile(filepath.Join(SystemdNetworkConfigPath, cfg.Name), []byte(cfg.Contents), 0o600)
		if err != nil {
			return err
		}
	}

	// Generate .netdev files.
	for _, cfg := range GenerateNetdevFileContents(networkCfg) {
		err := os.WriteFile(filepath.Join(SystemdNetworkConfigPath, cfg.Name), []byte(cfg.Contents), 0o600)
		if err != nil {
			return err
		}
	}

	// Generate .network files.
	for _, cfg := range GenerateNetworkFileContents(networkCfg) {
		err := os.WriteFile(filepath.Join(SystemdNetworkConfigPath, cfg.Name), []byte(cfg.Contents), 0o600)
		if err != nil {
			return err
		}
	}

	return nil
}

// ApplyNetworkConfiguration instructs systemd-networkd to apply the supplied network configuration.
func ApplyNetworkConfiguration(ctx context.Context, networkCfg NetworkConfig) error {
	err := GenerateNetworkConfiguration(networkCfg)
	if err != nil {
		return err
	}

	// Trigger udev rule update to pickup device names.
	err = RestartUnit(ctx, "systemd-udev-trigger")
	if err != nil {
		return err
	}

	// Restart networking after new config files have been generated.
	return RestartUnit(ctx, "systemd-networkd")
}

// Helper function that generates the contents of systemd.link files. Returns an array of configFile structs.
// https://www.freedesktop.org/software/systemd/man/latest/systemd.link.html
func GenerateLinkFileContents(networkCfg NetworkConfig) []ConfigFile {
	ret := []ConfigFile{}

	for _, i := range networkCfg.Interfaces {
		strippedHwaddr := strings.ReplaceAll(i.Hwaddr.String(), ":", "")
		ret = append(ret, ConfigFile{
			Name: fmt.Sprintf("00-en%s.link", strippedHwaddr),
			Contents: fmt.Sprintf(`[Match]
MACAddress=%s

[Link]
NamePolicy=
Name=en%s
`, i.Hwaddr.String(), strippedHwaddr),
		})
	}

	return ret
}

// Helper function that generates the contents of systemd.netdev files. Returns an array of configFile structs.
// https://www.freedesktop.org/software/systemd/man/latest/systemd.netdev.html
func GenerateNetdevFileContents(networkCfg NetworkConfig) []ConfigFile {
	ret := []ConfigFile{}

	// Create bridge devices for each interface.
	for _, i := range networkCfg.Interfaces {
		ret = append(ret, ConfigFile{
			Name: fmt.Sprintf("00-%s.netdev", i.Name),
			Contents: fmt.Sprintf(`[NetDev]
Name=%s
Kind=bridge
MACAddress=%s

[Bridge]
VLANFiltering=true
`, i.Name, i.Hwaddr.String()),
		})
	}

	// Create bonds.
	for _, b := range networkCfg.Bonds {
		ret = append(ret, ConfigFile{
			Name: fmt.Sprintf("00-bn%s.netdev", b.Name),
			Contents: fmt.Sprintf(`[NetDev]
Name=bn%s
Kind=bond
MTUBytes=%d

[Bond]
Mode=%s
`, b.Name, b.MTU, b.Mode),
		})
	}

	// Create vlans.
	for _, v := range networkCfg.Vlans {
		ret = append(ret, ConfigFile{
			Name: fmt.Sprintf("00-vl%s.netdev", v.Name),
			Contents: fmt.Sprintf(`[NetDev]
Name=vl%s
Kind=vlan
MTUBytes=%d

[Bridge]
Id=%d
`, v.Name, v.MTU, v.ID),
		})
	}

	return ret
}

// Helper function that generates the contents of systemd.network files. Returns an array of configFile structs.
// https://www.freedesktop.org/software/systemd/man/latest/systemd.network.html
func GenerateNetworkFileContents(networkCfg NetworkConfig) []ConfigFile {
	ret := []ConfigFile{}

	// Create networks for each interface.
	for _, i := range networkCfg.Interfaces {
		cfgString := fmt.Sprintf(`[Match]
Name=%s

[Network]
`, i.Name)

		cfgString += processAddresses(i.Addresses)

		if len(i.Routes) > 0 {
			cfgString += "\n[Route]\n"
			cfgString += processRoutes(i.Routes)
		}

		if i.VLAN != 0 {
			cfgString += fmt.Sprintf("\n[BridgeVLAN]\nVLAN=%d\n", i.VLAN)
		}

		ret = append(ret, ConfigFile{
			Name:     fmt.Sprintf("00-%s.network", i.Name),
			Contents: cfgString,
		})

		ret = append(ret, ConfigFile{
			Name: fmt.Sprintf("00-%s-dev0.network", i.Name),
			Contents: fmt.Sprintf(`[Match]
Name=en%s

[Network]
Bridge=%s
`, strings.ReplaceAll(i.Hwaddr.String(), ":", ""), i.Name),
		})
	}

	// Create networks for each bond.
	for _, b := range networkCfg.Bonds {
		cfgString := fmt.Sprintf(`[Match]
Name=bn%s

[Network]
`, b.Name)

		cfgString += processAddresses(b.Addresses)

		if len(b.Routes) > 0 {
			cfgString += "\n[Route]\n"
			cfgString += processRoutes(b.Routes)
		}

		if b.VLAN != 0 {
			cfgString += fmt.Sprintf("\n[BridgeVLAN]\nVLAN=%d\n", b.VLAN)
		}

		ret = append(ret, ConfigFile{
			Name:     fmt.Sprintf("00-bn%s.network", b.Name),
			Contents: cfgString,
		})

		for index, member := range b.Members {
			ret = append(ret, ConfigFile{
				Name: fmt.Sprintf("00-bn%s-dev%d.network", b.Name, index),
				Contents: fmt.Sprintf(`[Match]
MACAddress=%s

[Network]
Bond=bn%s
`, member.String(), b.Name),
			})
		}
	}

	return ret
}

func processAddresses(addresses []CustomAddress) string {
	ret := ""

	hasDHCP4 := false
	hasDHCP6 := false
	for _, addr := range addresses {
		switch addr.Type {
		case AddressTypeDHCP4:
			hasDHCP4 = true
		case AddressTypeDHCP6:
			hasDHCP6 = true
			ret += "IPv6AcceptRA=false\n"
		case AddressTypeSLAAC:
			ret += "IPv6AcceptRA=true\n"
		default:
			ret += fmt.Sprintf("Address=%s\n", addr.String())
		}
	}

	if hasDHCP4 && hasDHCP6 {
		ret += "DHCP=yes"
	} else if hasDHCP4 {
		ret += "DHCP=ipv4"
	} else if hasDHCP6 {
		ret += "DHCP=ipv6"
	}

	return ret
}

func processRoutes(routes []Route) string {
	ret := ""

	for _, route := range routes {
		switch route.Via.Type {
		case AddressTypeDHCP4:
			ret += "Gateway=_dhcp4\n"
		case AddressTypeSLAAC:
			ret += "Gateway=_ipv6ra\n"
		default:
			ret += fmt.Sprintf("Gateway=%s\n", route.Via.String())
		}

		ret += fmt.Sprintf("Destination=%s\n", route.To.String())
	}

	return ret
}
