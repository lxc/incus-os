...
func TestNetworkConfigMarshalling(t *testing.T) {
	var cfg, cfgAgain systemd.NetworkConfig
...
func TestLinkFileGeneration(t *testing.T) {
	var networkCfg systemd.NetworkConfig

	err := yaml.Unmarshal([]byte(networkdConfig1), &networkCfg)
	require.NoError(t, err)

	cfgs := systemd.GenerateLinkFileContents(networkCfg)
	...
	networkCfg = systemd.NetworkConfig{}
...
func TestNetdevFileGeneration(t *testing.T) {
	...
	cfgs := systemd.GenerateNetdevFileContents(networkCfg)
...
func TestNetworkFileGeneration(t *testing.T) {
	...
	require.Equal(t, "[Match]\nName=management\n\n[Network]\nIPv6AcceptRA=true\nDHCP=ipv4\n[Route]\nGateway=_dhcp4\nDestination=0.0.0.0/0\nGateway=_ipv6ra\nDestination=::/0\n", cfgs[0].Contents)
	require.Equal(t, "00-management-dev0.network", cfgs[1].Name)
	require.Equal(t, "[Match]\nName=enaabbccddee01\n\n[Network]\nBridge=management\n", cfgs[1].Contents)
}
