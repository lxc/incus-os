...
System.Network.Config.NTP.Timeservers[0]: ntp.example.org
...
System.Update.Config.CheckFrequency: 6h0m0s
System.Storage.Config.ScrubSchedule: 0 4 * * 0
System.Storage.Config.TrimSchedule: 0 4 * * 6
`

var goldEncodingV10 = `#Version: 10
Applications.Incus.State.Initialized: true
Applications.Incus.State.Version: 202506241635
OS.Name: IncusOS
OS.RunningRelease: 202506241635
...
func TestDottedMapKey(t *testing.T) {
	...
	s.System.Provider.Config.Config = map[string]string{
		"dotted.key": "value",
	}
	...
	require.Contains(t, string(contents), "dotted__DOT__key")
	...
	require.Equal(t, s.System.Provider.Config.Config["dotted.key"], newS.System.Provider.Config.Config["dotted.key"])
}
...
func TestCustomEncoding(t *testing.T) {
	...
	for _, goldVersion := range []string{goldEncodingV0, goldEncodingV1, goldEncodingV2, goldEncodingV3, goldEncodingV4, goldEncodingV5, goldEncodingV6, goldEncodingV7, goldEncodingV8, goldEncodingV9} {
		var s state.State
		...
		require.Equal(t, goldEncodingV10, string(content))
		require.Equal(t, 10, s.StateVersion)
...
func TestUpgradeFuncs(t *testing.T) {
	...
	err := state.Decode([]byte(goldEncodingV7), funcs, &s)
	require.NoError(t, err)

	require.Equal(t, 9, s.StateVersion)
