...
// IncusOSManifestDiff represents the differences between two IncusOS manifests.
type IncusOSManifestDiff struct {
	PreviousVersion string                    `json:"previous_version"`
	CurrentVersion  string                    `json:"current_version"`
	Added           []MkosiManifestPackages   `json:"added"`
	Removed         []MkosiManifestPackages   `json:"removed"`
	Updated         [][]MkosiManifestPackages `json:"updated"`
}
...
func DiffManifests(a IncusOSManifest, b IncusOSManifest) (IncusOSManifestDiff, error) {
	ret := IncusOSManifestDiff{
		PreviousVersion: a.Config.Version,
		CurrentVersion:  b.Config.Version,
	}

	// Generate diff for mkosi-installed packages.
	ret.Added, ret.Removed, ret.Updated = diffPackages(a.Packages, b.Packages)
	...
	for _, previousArtifact := range a.Artifacts {
		...
		if index == -1 {
			...
			ret.Removed = append(ret.Removed, MkosiManifestPackages{
			...
			ret.Updated = append(ret.Updated, []MkosiManifestPackages{
				{
					Name:    previousArtifact.Name,
					Version: previousArtifact.Version,
				},
	...
	for _, currentArtifact := range b.Artifacts {
		...
		if index == -1 {
			...
			ret.Added = append(ret.Added, MkosiManifestPackages{
...
// RenderDiff generates a human readable, yaml-compliant list of the changes.
func RenderDiff(d IncusOSManifestDiff) (string, error) {
	var sb strings.Builder

	for _, a := range d.Added {
		_, err := sb.WriteString("- Added " + a.Name + " version " + a.Version + "\n")
		if err != nil {
			return "", err
		}
	}
	...
	for _, r := range d.Removed {
		_, err := sb.WriteString("- Removed " + r.Name + " version " + r.Version + "\n")
		if err != nil {
			return "", err
		}
	}
	...
	for _, u := range d.Updated {
		_, err := sb.WriteString("- Updated " + u[0].Name + " from version " + u[0].Version + " to version " + u[1].Version + "\n")
		if err != nil {
			return "", err
		}
	}
...
// manifest. We always exist a base manifest to exist and be the first element in the list of manifests.
...
func WriteManifests(root string, manifests map[string]IncusOSManifest) error {
	...
	for manifest := range manifests {
		content, err := json.MarshalIndent(manifests[manifest], "", "    ")
		if err != nil {
			return err
		}

		err = os.WriteFile(filepath.Join(root, manifest+".json"), content, 0o644)
		if err != nil {
			return err
		}
	}
