...
func GetEmbeddedCertificates() (Certificates, error) {
	...
	contents, err := files.ReadFile("production-cert-subject-key-ids.txt")
	if err != nil {
	...
	rootCAContents, err := files.ReadFile("root-E1.crt")
	if err != nil {
		return parsedCerts, err
	}
	...
	secureBootCAContents, err := files.ReadFile("secureboot-E1.crt")
	if err != nil {
		return parsedCerts, err
	}
	...
	contents, err = files.ReadFile("update-E1.crt")
	if err != nil {
		return parsedCerts, err
	}
	...
	contents, err = files.ReadFile("secureboot-PK-R1.crt")
	if err != nil {
		return parsedCerts, err
	}
	...
	for _, cert := range certs {
		if !strings.HasPrefix(cert.Name(), "secureboot-KEK-") {
			continue
		}

		contents, err := files.ReadFile(cert.Name())
		if err != nil {
			return parsedCerts, err
		}
	...
	for _, cert := range certs {
		if !strings.HasPrefix(cert.Name(), "secureboot-DB-") {
			continue
		}
	...
	for _, cert := range certs {
		if !strings.HasPrefix(cert.Name(), "secureboot-DBX-") {
			continue
		}
