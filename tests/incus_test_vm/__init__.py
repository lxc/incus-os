...
class IncusTestVM:
    ...
    def __enter__(self):
        ...
        subprocess.run(["incus", "init", "--empty", "--vm", self.vm_name, "-c", "security.secureboot=false", "-c", "limits.cpu=4", "-c", "limits.memory=8GiB", "-d", "root,size="+self.root_size], capture_output=True, check=True)
        subprocess.run(["incus", "config", "device", "add", self.vm_name, "vtpm", "tpm"], capture_output=True, check=True)

        if self.is_raw_image:
            subprocess.run(["incus", "config", "device", "add", self.vm_name, "boot-media", "disk", "source="+self.install_image, "io.bus=usb", "boot.priority=10", "readonly=false"], capture_output=True, check=True)
        else:
            subprocess.run(["incus", "config", "device", "add", self.vm_name, "boot-media", "disk", "pool=default", "source="+self.vm_name+".iso", "boot.priority=10"], capture_output=True, check=True)
    ...
    def WaitSystemReady(self, incusos_version, source="/dev/sdb", target="/dev/sda", application="incus"):
        """Wait for the system install to complete, the given application to be configured and the become system ready for use."""
    ...
    def WaitAgentRunning(self, timeout=60):
